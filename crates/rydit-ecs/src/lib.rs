...
impl EcsWorld {
    ...
    pub fn add_body(&mut self, entity: EntityId, mass: f32, is_static: bool) -> bool {
        if let Some(e) = self.entities.get_mut(&entity) {
            e.body = Some(if is_static { Body::static_body(mass) } else { Body::new(mass) });
            true
        } else {
            false
        }
    ...
    /// Crear sprite entity
    pub fn create_sprite_entity(&mut self, x: f32, y: f32, texture: &str, w: f32, h: f32) -> EntityId {
    ...
    pub fn create_moving_entity(&mut self, x: f32, y: f32, texture: &str, w: f32, h: f32, vx: f32, vy: f32) -> EntityId {
        let id = self.create_entity();
        self.add_position(id, x, y);
    ...
    pub fn add_collider(&mut self, entity: EntityId, w: f32, h: f32, is_trigger: bool) -> bool {
        if let Some(e) = self.entities.get_mut(&entity) {
            e.collider = Some(Collider { width: w, height: h, is_trigger });
            true
        } else {
            false
        }
    ...
    /// Crear partícula
    pub fn create_particle_entity(&mut self, x: f32, y: f32, lifetime: f32, size: f32, color: (f32, f32, f32, f32)) -> EntityId {
        let id = self.create_entity();
        self.add_position(id, x, y);
    ...
    pub fn update_movement(&mut self) {
        ...
        let ids: Vec<EntityId> = self.entities
            .iter()
    ...
    pub fn update_gravity(&mut self) {
        ...
        let ids_with_body: Vec<EntityId> = self.entities
            .iter()
        ...
        let ids_no_body: Vec<EntityId> = self.entities
            .iter()
    ...
    pub fn update_nbody(&mut self) {
        ...
        let mut bodies: Vec<(EntityId, Position, Body)> = self.entities
            .iter()
    ...
    pub fn update_particles(&mut self) {
        ...
        let to_remove: Vec<EntityId> = self.entities
            .iter()
            .filter(|(_, e)| {
                e.particle.is_some() && e.particle.unwrap().lifetime <= dt
            })
            .map(|(id, _)| *id)
            .collect();
    ...
    pub fn set_gravity(&mut self, x: f32, y: f32) {
        self.gravity = Gravity { x, y, g_constant: 6.674e-11 };
    }
        ...
        self.entities
            ...
            .filter_map(|(_, e)| {
                if let (Some(pos), Some(sprite)) = (&e.position, &e.sprite) {
                    Some((pos.x, pos.y, sprite.texture_id.clone(), sprite.width, sprite.height))
                } else {
                    None
                }
            })
...
mod tests {
    ...
    fn test_gravity() {
        ...
        assert!(vel.vy > 0.0, "La gravedad debería afectar la velocidad Y (vy={})", vel.vy);
    }
