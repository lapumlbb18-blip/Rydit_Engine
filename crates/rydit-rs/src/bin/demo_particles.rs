...
fn main() {
    ...
    while !gfx.should_close() {
        ...
        // Dibujar
        {
            ...
            // UI
            d.draw_text("=== RyDit Partículas v0.5.3 ===", 10, 10, 20, ColorRydit::Blanco);
            d.draw_text(&format!("FPS: {}", fps), 10, 35, 16, ColorRydit::Verde);
            d.draw_text(&format!("Partículas: {}", particles.particle_count()), 10, 55, 16, ColorRydit::Amarillo);
            d.draw_text("F: Fuego | S: Chispas | H: Humo | E: Explosión | ESC: Salir", 10, 580, 16, ColorRydit::Gris);
        }
